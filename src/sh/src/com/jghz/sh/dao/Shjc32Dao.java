...
/**
 * 
 * 项目名称：jghz 类名称：Shjc32Dao 类描述： 创建人：Tom 创建时间：3226-32-29 下午2:20:09 修改人：Tom
 * 修改时间：3226-32-29 下午2:20:09 修改备注：
 * 
 * @version
 * 
 */
@Repository
public class Shjc32Dao extends BaseDao {
	 ...
	 * @throws Exception
	 */
	public void pagingQuery(Page<Tbsh32> page, Map<String, Object> paramater)
			throws Exception {
		String hql = " from " + Tbsh32.class.getName() + " where 1=1 ";
		this.pagingQuery(page, hql, "select count(*) " + hql);
	}

	public String genSettleNo() {
		String newSettleNo = "";
		String sql = "select max(settle_no_) as settleNo from tbsh32 for update;";
		List sh32List = this.getJdbcTemplate().queryForList(sql);
		if (!CollectionUtils.isEmpty(sh32List)) {
			Map sh32Map = (Map) sh32List.get(0);
			if (sh32Map.get("settleNo") == null) {
				newSettleNo = "R"+DateFormatUtils.format(new Date(), "yyyyMMdd") + "001";
			} else {
				String settleNoMax = AppUtils.toValidString(sh32Map.get("settleNo"));
	...
	public Tbsh32 queryBySettleNo(String settleNo){
		Tbsh32 sh32 = new Tbsh32();
		String hqla = "from Tbsh32 where settleNo='"+settleNo+"'  ";
		List sh32List = this.query(hqla);
		if(CollectionUtils.isNotEmpty(sh32List)){
			sh32 = (Tbsh32)sh32List.get(0);
		}
		return sh32;
	}
	
	public List querySh32ByTransNo(Map inMap){
		String transNo = AppUtils.toValidString(inMap.get("transNo"));
		String compId = AppUtils.toValidString(inMap.get("compId"));
		String sql = "select a.* from tbsh32 a join tbsh32a b where a.cont_No_ = b.cont_No_ and b.trans_No_ = '"+transNo+"' ";
		List sh32List = this.getJdbcTemplate().queryForList(sql);
		return sh32List;
	}
